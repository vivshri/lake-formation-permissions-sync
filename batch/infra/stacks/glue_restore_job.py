from aws_cdk import Stack, aws_glue, aws_iam, aws_s3, aws_s3_deployment
...
class GlueRestoreOnDemandStack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        # Glue and Lake Formation permissions for cross-region catalog replication.
        # The role needs both IAM permissions AND Lake Formation grants.
        # AWSGlueServiceRole covers basic Glue in the source region, but the job
        # also creates/updates databases, tables, and partitions in the target region
        # and manages Lake Formation permissions in both regions.
        ...
        aws_glue.CfnJob(
            self,
            "LFRestoreOnDemandGlueJob",
            role=glue_role.role_arn,
            command=aws_glue.CfnJob.JobCommandProperty(
                name="glueetl", python_version="3", script_location="s3://" + script_bucket.bucket_name + "/app.py"
            ),
            default_arguments={
